#!/usr/bin/env python3
"""
MEXC Scanner V3 - Telegram Bot Controller
Scalping-optimized bot with perpetual futures support
"""
...
SCANNER_SCRIPT = os.path.expanduser('~/trading/scripts/mexc_pro_scannerV3.py')
RESULTS_JSON = os.path.expanduser('~/trading/data/scan_results_v3.json')
STATE_FILE = os.path.expanduser('~/trading/data/bot_state_v3.json')
...
class BotState:
    def __init__(self):
        ...
        self.min_threshold = 75  # V3 uses 75/110 minimum
        ...
        try:
            if os.path.exists(STATE_FILE):
                with open(STATE_FILE, 'r') as f:
                    ...
                    self.min_threshold = data.get('min_threshold', 75)
...
def send_message(text: str, parse_mode: str = 'Markdown', keyboard: bool = False) -> bool:
...
def cmd_settings(args):
    ...
    focus_status = f"{len(state.focus_pairs)} pair(s)" if state.focus_pairs else "All pairs"
    ...
    msg += f"• `/focus <symbol>` - Add pair to focus\n"
    msg += f"• `/clear_focus` - Remove all focus\n"
    msg += f"• `/mute` - Disable notifications\n"
    msg += f"• `/unmute` - Enable notifications\n\n"

    msg += f"*Examples:*\n"
    msg += f"• `/interval 120` - Scan every 2 minutes\n"
    msg += f"• `/threshold 80` - Only show 80+ scores\n"
    msg += f"• `/focus BTC` - Focus on Bitcoin\n"
...
def cmd_stats(args):
    ...
    paper_trading_file = os.path.expanduser('~/trading/data/paper_trading_v3.json')
    
    if not os.path.exists(paper_trading_file):
        return "📊 No paper trading data available"
    ...
    try:
        ...
        if not closed and not open_trades:
            return "📊 No trades yet\n\nWait for scanner to find signals"
        ...
        if closed:
            ...
            for t in closed[-5:]:
                ...
                status = t.get('status', '?')
                msg += f"{icon} {symbol} {direction}: {pnl:.2f}% ({status})\n"
        
        return msg
        
    except Exception as e:
        return f"❌ Error reading paper trading data: {e}"
...
def cmd_check(args):
    ...
    if not args:
        return ("*🔍 Check Position*\n\n"
                "Analyze ANY pair you're holding - even if it doesn't meet signal criteria.\n\n"
                "*Usage:*\n"
                "`/check BTC/USDT:USDT`\n"
                "`/check ETH` (auto-completes to ETH/USDT:USDT)\n"
                "`/check 1` (check pair #1 from last scan)\n\n"
                "*Shows:*\n"
                "• Current trend on all timeframes\n"
                "• Technical indicators\n"
                "• Risk assessment\n"
                "• Hold/Exit guidance\n\n"
                "⚡ Works even if pair is ranging or doesn't meet criteria!")
    ...
    try:
        ...
        from mexc_pro_scannerV3 import ScalpingScanner, Config as ScannerConfig
        import logging
...
def cmd_help(args):
    """Show help"""
    return """*MEXC Scanner V3 Bot Commands*
...
*Position Analysis:*
/check <symbol> - Analyze specific pair (e.g., BTC/USDT:USDT)
/check <number> - Check pair #N from last scan
/check - Show usage and examples
...
*Settings:*
/threshold <score> - Min score (default: 75/110)
...
• 1h trend filter (required)
• 4h trend bonus
• Near misses display
• Position analysis with hold/exit guidance
• Scalping-optimized (5min max hold)
"""

def cmd_ping(args):
    """Ping command"""
    return "🏓 Pong! Scanner V3 Bot is alive."
...
COMMANDS = {
    ...
    '📊 scan': cmd_scan,
    '▶️ start live': cmd_start_live,
    '⏹️ stop live': cmd_stop_live,
    '📈 status': cmd_status,
    '🎯 signals': cmd_signals,
    '🔍 check': cmd_check,
