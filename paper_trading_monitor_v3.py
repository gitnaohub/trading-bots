#!/usr/bin/env python3
"""
Paper Trading Monitor V3 - Tracks V3 signals and simulates trades
Runs alongside V3 scanner, updates trades with live prices
"""
...
from mexc_pro_scannerV3 import ScalpingScanner

# Configuration
RESULTS_JSON = os.path.expanduser('~/trading/data/scan_results_v3.json')
PAPER_TRADING_DATA = os.path.expanduser('~/trading/data/paper_trading_v3.json')
LAST_PROCESSED_FILE = os.path.expanduser('~/trading/data/paper_trading_v3_last_scan.txt')
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f'paper_trading_v3_{datetime.now().strftime("%Y%m%d")}.log')
...
def check_new_signals(tracker: PaperTradingTracker, logger):
    """Check for new signals from V3 scanner and add to paper trading"""
    try:
        ...
        for signal in qualified:
            ...
            logger.info(f"📝 V3 paper trade: {signal['symbol']} {signal['direction']} @ ${signal['entry_price']:.2f} (Score: {signal['score']})")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PAPER TRADING MONITOR V3 - STARTED")
    logger.info("=" * 80)

    # Initialize tracker
    tracker = PaperTradingTracker(PAPER_TRADING_DATA)
    logger.info(f"📊 Loaded V3 paper trading: {len(tracker.open_trades)} open, {len(tracker.closed_trades)} closed")
    ...
    try:
        while True:
            ...
            if tracker.open_trades:
                ...
                for trade in closed_trades:
                    ...
                    logger.info(f"{status_icon} V3 closed: {trade.symbol} {trade.direction} | "
        ...
        logger.info("\n⏹️  V3 paper trading monitor stopped")
        stats = tracker.get_stats()
        logger.info(f"📊 V3 final: {stats['total_trades']} trades, {stats['win_rate']:.1f}% WR, {stats['total_pnl_pct']:+.2f}% PnL")
    except Exception as e:
